...
function SkipLinks(options) {
  ...
  var DO_SKIP_LINKS = options.enabled === undefined ? true : options.enabled,
    ...
    SKIP_LINK_TARGET_SELECTOR =
      options.skip_link_target_selector || ".accessibilityWrapper",
  ...
  function buildSkipLinks() {
    ...
    for (var i = 0; i < NUM_SLIDES; i++) {
      ...
      skipLinkHTML +=
        '<li><a href="#/' +
        SLIDES[i].getAttribute("data-id") +
        '">' +
        (i + 1) +
        ". " +
        sanitizeText(slideText) +
        "</a></li>"
    }
    skipLinkHTML += "</ul>"

    dom.skipLinks = createNodeAfterSibling(
      dom.wrapper,
      "ul",
      SLIDE_SKIP_LINKS_ID,
      skipLinkHTML,
   ...
   * inserted after.
   */
  function createNodeAfterSibling(
    container,
    tagname,
    id,
    innerHTML,
